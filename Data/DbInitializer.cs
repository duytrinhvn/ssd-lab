...
namespace Assignment1.Data
{
    public static class DbInitializer
    {
        ...
        {
            // Create Admin User
            var managernUser = new ApplicationUser
            {
            ...
            var result = await userManager.CreateAsync(managernUser, appSecrets.ManagerPwd);
            if (!result.Succeeded)
                return 1;  // should log an error message here

            // Assign user to Admin role
            result = await userManager.AddToRoleAsync(managernUser, "Manager");
            if (!result.Succeeded)
                return 2;  // should log an error message here
